...
<div class="max-w-2xl mx-auto bg-white rounded-xl shadow-sm p-8">
    <div class="flex justify-between items-center mb-6 border-b pb-4">
        <h2 class="text-xl font-bold text-gray-800">일정 수정</h2>
    ...
    <form action="{{ route('admin.schedules.update', $schedule->id) }}" method="POST">
        ...
        <div class="mb-5">
            ...
            <input type="text" name="title" value="{{ old('title', $schedule->title) }}" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 outline-none" required>
        </div>

        <div class="grid grid-cols-2 gap-4 mb-5">
            <div>
                <label class="block text-gray-700 text-sm font-bold mb-2">시작일 <span class="text-red-500">*</span></label>
                <input type="date" name="start" value="{{ old('start', $schedule->start->format('Y-m-d')) }}" class="w-full border border-gray-300 rounded-lg px-4 py-2" required>
            </div>
            <div>
                <label class="block text-gray-700 text-sm font-bold mb-2">종료일 (선택)</label>
                <input type="date" name="end" value="{{ old('end', $schedule->end ? $schedule->end->format('Y-m-d') : '') }}" class="w-full border border-gray-300 rounded-lg px-4 py-2">
            </div>
        </div>
        ...
        <div class="mb-5">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                <label class="cursor-pointer">
                    <input type="radio" name="color" value="{{ $color['code'] }}" class="peer sr-only" 
                        {{ (old('color', $schedule->color) == $color['code']) ? 'checked' : '' }}>
        ...
        <div class="mb-8">
            <label class="flex items-center cursor-pointer">
                <input type="checkbox" name="is_display" value="1" {{ $schedule->is_display ? 'checked' : '' }} class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
        ...
        <div class="flex justify-end gap-3 pt-4 border-t">
            ...
            <button type="submit" class="px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium shadow-md transition">수정 완료</button>
        </div>
    </form>
</div>
@endsection