...
@section('content')
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 max-w-4xl mx-auto">
        ...
        <form action="{{ route('admin.notice.update', $notice->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT') {{-- 수정 시 필수 --}}
            ...
            <div class="w-1/3">
                ...
                <input type="text" name="writer" id="writer" required value="{{ old('writer', $notice->writer) }}"
                       class="w-full border-gray-300 rounded-md shadow-sm focus:ring-amber-500 focus:border-amber-500 px-4 py-2 border">
            </div>

            {{-- 옵션 --}}
            <div class="flex items-center gap-2">
                <input type="checkbox" name="is_display" id="is_display" value="1" {{ $notice->is_display ? 'checked' : '' }}
            ...
            <div class="bg-gray-50 p-4 rounded-md border border-gray-200">
                ...
                @if($notice->file_path)
                    <div class="flex items-center justify-between bg-white p-3 border rounded mb-3">
                        <div class="flex items-center gap-2">
                            <i class="xi-file-o text-gray-500"></i>
                            <a href="{{ asset('storage/'.$notice->file_path) }}" target="_blank" class="text-blue-600 hover:underline text-sm">
                                현재 파일: {{ basename($notice->file_path) }}
                            </a>
                        </div>
                        <div class="flex items-center gap-1">
                            <input type="checkbox" name="delete_file" id="delete_file" value="1" class="h-4 w-4 text-red-600">
                            <label for="delete_file" class="text-xs text-red-500 cursor-pointer">삭제</label>
                        </div>
                    </div>
                @endif
                ...
                <input type="file" name="file" class="block w-full text-sm text-gray-500
                  ...
                  file:bg-white file:text-gray-700 file:border file:border-gray-300
                  hover:file:bg-gray-50
                "/>
                <p class="text-xs text-gray-500 mt-2">※ 파일을 새로 선택하면 기존 파일은 자동으로 교체됩니다.</p>
            </div>
            ...
            <div class="flex justify-end gap-2">
                ...
                <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-md hover:bg-blue-700 transition font-medium">수정 저장</button>
            </div>
